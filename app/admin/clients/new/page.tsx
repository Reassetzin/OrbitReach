      ...
      <div style={{ padding:'32px', maxWidth:560 }}>
        <form onSubmit={handleSubmit} style={{ background:'#fff', border:'1px solid #E8EAF0', borderRadius:16, padding:32, display:'flex', flexDirection:'column', gap:20 }}>
          {[
            ...
            <div key={f.key}>
              <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'#64748B', display:'block', marginBottom:6 }}>{f.label}</label>
              <input type={f.type} value={(form as any)[f.key]} onChange={e => update(f.key, e.target.value)} placeholder={f.placeholder} required={f.label.includes('*')}
                style={{ width:'100%', fontSize:13, padding:'10px 14px', border:'1.5px solid #E8EAF0', borderRadius:8, background:'#F5F6FA', outline:'none', boxSizing:'border-box' }}/>
            </div>
          ))}
          <div>
            <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'#64748B', display:'block', marginBottom:6 }}>Service type</label>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div>
              <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'#64748B', display:'block', marginBottom:6 }}>Setup fee ($)</label>
              <input type="number" value={form.setup} onChange={e => update('setup', e.target.value)} placeholder="e.g. 500"
                style={{ width:'100%', fontSize:13, padding:'10px 14px', border:'1.5px solid #E8EAF0', borderRadius:8, background:'#F5F6FA', outline:'none', boxSizing:'border-box' }}/>
            </div>
            <div>
              <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'#64748B', display:'block', marginBottom:6 }}>Monthly retainer ($)</label>
              <input type="number" value={form.retainer} onChange={e => update('retainer', e.target.value)} placeholder="e.g. 200"
                style={{ width:'100%', fontSize:13, padding:'10px 14px', border:'1.5px solid #E8EAF0', borderRadius:8, background:'#F5F6FA', outline:'none', boxSizing:'border-box' }}/>
            </div>
          </div>
          <div>
            <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'#64748B', display:'block', marginBottom:6 }}>Monthly revisions</label>
            <input type="number" value={form.revisions} onChange={e => update('revisions', e.target.value)}
              style={{ width:'100%', fontSize:13, padding:'10px 14px', border:'1.5px solid #E8EAF0', borderRadius:8, background:'#F5F6FA', outline:'none', boxSizing:'border-box' }}/>
          </div>
          <div>
            <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'#64748B', display:'block', marginBottom:8 }}>Initial status</label>
          ...
          <button type="submit" disabled={saving || saved}
            style={{ padding:'12px', background: saved ? 'linear-gradient(135deg,#10B981,#059669)' : 'linear-gradient(135deg,#6C63FF,#A855F7)', color:'#fff', border:'none', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer', boxShadow:'0 8px 24px rgba(108,99,255,.25)' }}>
            {saved ? '✓ Saved! Redirecting…' : saving ? 'Saving…' : 'Add client →'}
          </button>
        </form>
      </div>
    </div>
